...
process.env.NEXT_GOOGLE_API_KEY = 'test-google-key';
let sessionStopCalls = 0;
process.env.NEXT_GOOGLE_API_KEY = 'google-api-key';
...
async function verifyInviteAgentSuccess() {
  ...
  Agent.prototype.createSession = (function (
    this: Agent,
    ...
    capturedSttConfig = (
      this as unknown as {
        _stt?: { params?: Record<string, unknown> };
      }
    )._stt ?? null;
    return {
  ...
  try {
    ...
    assert(
      JSON.stringify(sttConfig?.params?.custom_vocabulary) ===
        JSON.stringify(['Agora', 'Gemini']),
      'POST /api/invite-agent should send the Gemini custom vocabulary',
    );
    assert(
      sttConfig?.params?.word_timestamp === false,
      'POST /api/invite-agent should explicitly disable word timestamps with custom vocabulary',
    );
  } finally {
...
async function verifyStopConversationSuccess() {
  ...
  assert(response.status === 200, 'POST /api/stop-conversation should return 200');
  assert(body.success === true, 'POST /api/stop-conversation should return success');
  assert(sessionStopCalls === 1, 'POST /api/stop-conversation should stop the retained session');
  ...
  assert(
    repeatedResponse.status === 200 && repeatedBody.state === 'not-found',
    'repeated stops should return an idempotent not-found response',
  );
}
