...
2. Add `env.local.example` with `NEXT_PUBLIC_AGORA_APP_ID`, `NEXT_AGORA_APP_CERTIFICATE`, required `NEXT_GOOGLE_API_KEY`, optional `NEXT_PUBLIC_AGENT_UID`, optional `NEXT_AGENT_GREETING`, and optional custom-LLM extension examples.
...
4. Implement `POST /api/invite-agent` with standard `AgoraClient`, `Agent`, GeminiSTT (`gemini-3.5-transcribe-live`), Gemini (`gemini-3.6-flash`), MiniMaxTTS (`en-US-Chirp3-HD-Charon`, `en-US`, 24000 Hz), one shared `NEXT_GOOGLE_API_KEY`, RTM enabled, metrics enabled, and `{ requester_id, channel_name }` input. Store the started `AgentSession` in a shared process-local registry and stop only through that retained session.
5. Implement `POST /api/stop-conversation` by taking the retained session, calling `session.stop()`, and returning idempotent `not-found` for missing IDs.
