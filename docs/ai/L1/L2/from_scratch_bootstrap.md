...
4. Implement `POST /api/invite-agent` with standard `AgoraClient`, `Agent`, GeminiSTT (`gemini-3.5-transcribe-live`), Gemini (`gemini-3.6-flash`), GeminiTTS (`gemini-3.8-flash-tts`, voice `Puck`), one shared `NEXT_GOOGLE_API_KEY`, RTM enabled, metrics enabled, and `{ requester_id, channel_name }` input. Store the started `AgentSession` in a shared process-local registry and stop only through that retained session.
