...
| --- | --- | --- | --- |
| Agent never joins | Invite route or env mismatch | `pnpm run doctor` and invite route logs | Verify `NEXT_GOOGLE_API_KEY`, `NEXT_PUBLIC_AGENT_UID`, and invite payload |
...
- Keep `NEXT_AGORA_APP_CERTIFICATE` and `NEXT_GOOGLE_API_KEY` private server variables.
