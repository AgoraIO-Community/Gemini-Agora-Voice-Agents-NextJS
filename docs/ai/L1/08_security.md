...
- Do not expose `NEXT_GOOGLE_API_KEY` or optional custom-provider keys to client bundles.
...
- `NEXT_GOOGLE_API_KEY` set as a server-side secret; optional custom-provider keys set only when their path is wired.
